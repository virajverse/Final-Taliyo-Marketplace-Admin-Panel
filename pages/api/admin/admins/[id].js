...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      await auditAction(req, ok, 'update', 'admins', id, null, data)
    ...
    if (req.method === 'DELETE') {
      ...
      const { error } = await supabase
        ...
        .eq('id', id)
      if (error) throw error
      await auditAction(req, ok, 'delete', 'admins', id, null, null)
    ...
    return res.status(500).json({ error: 'admin_failed', message: e?.message })
  }
}
